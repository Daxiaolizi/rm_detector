...
void Inferencer::detect(Mat& frame)
{
  ...
  int BatchSize_ = batch_images.size();
  target_objects_.resize(BatchSize_);
  cuda_batch_preprocess(batch_images, device_buffers_[0], kInputW_, kInputH_, stream_);
  infer(*context_, stream_, (void**)device_buffers_, BatchSize_, decode_ptr_host_, decode_ptr_device_, model_bboxes_);
  batch_process(target_objects_, decode_ptr_host_, BatchSize_, bbox_element, batch_images);
}
